...
| 10-01-02 | 01 | 1 | INTRO-01, INTRO-02 | T-10-01-A, T-10-01-B, P-10-VISUAL, P-10-ACCESS | Besides deterministic injected seeking, normal playback proves primary/secondary inert only while invisible, primary unlock before secondary/CTAs, no invisible focusable CTA, visible skip/topbar availability, and immediate fully visible/non-inert reduced-motion, bypass and forced-failure states | build + focused visual/accessibility | `npm run build && npx playwright test tests/cinematic-intro-visual.spec.ts --project=emulated-chromium-desktop` | ❌ visual spec and PNGs are created by this task; `playwright.config.ts` already serves `npm run preview` | ⬜ pending |
| 10-01-03 | 01 | 1 | INTRO-01 | P-10-VISUAL | Editorial quality, continuity, light/reflection bridge, mobile direction and camera reading are explicitly approved | blocking human checkpoint | Review both generated contact sheets; all five criteria must receive explicit approval | ❌ waits on 10-01-02 artifacts | ⬜ pending |
| 10-02-01 | 02 | 2 | INTRO-01, INTRO-02 | T-10-02-B | Pure arc/progress/rate contracts are finite, responsive and aligned with the approved design | unit + build | `npm test -- src/lib/cinematicIntro.test.ts && npm run build` | ✅ unit spec exists; revised cases written in task | ⬜ pending |
| 10-02-02 | 02 | 2 | INTRO-01, INTRO-02 | T-10-02-A, T-10-02-D | Motion/lifecycle behavior is written red first, then one controller passes arc, resize, intent, reduce and fail-open contracts | focused Chromium + build | `npx playwright test tests/cinematic-intro.spec.ts --project=emulated-chromium-desktop -g "continuous\|arc\|geometry\|resize\|intent\|reduced motion\|fail-open" && npm run build` | ✅ focused spec exists; task adds revised cases before production | ⬜ pending |
| 10-02-03 | 02 | 2 | INTRO-01, INTRO-02 | T-10-02-A | Remaining semantic geometry and fault contracts complete the approved timeline matrix | focused Chromium | `npx playwright test tests/cinematic-intro.spec.ts --project=emulated-chromium-desktop -g "continuous\|arc\|geometry\|resize\|intent\|reduced motion\|fail-open"` | ✅ same focused spec, extended after 10-02-02 | ⬜ pending |
...
|------------|--------|-------------------|
| 10-01-03 — art direction | All of 10-02 and 10-03 | Explicit verdict for editorial first frame, landscape continuity, halo/reflection bridge, separate mobile direction and camera-versus-zoom, using both 0/40/70/88/100 contact sheets |
...
- During successful playback, each copy group is inert only until its own first visible frame; primary unlocks before secondary/CTAs, and no invisible CTA is focusable.
- Reduced motion, ineligible frequency/known-fragment entry and any provisional controller failure synchronously expose both copy groups fully visible, non-inert and operable.
