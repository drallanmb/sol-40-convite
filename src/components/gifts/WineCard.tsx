...
export function WineCard({
  ...
  return (
    ...
    >
      {selected ? (
        <span className="absolute left-6 top-4 z-[2] bg-cellar px-2 py-1 text-caption font-bold uppercase leading-label-relaxed tracking-label text-peach">
          {GIFTS_COPY.selected}
        </span>
      ) : null}
      ...
      <div className="mt-4 flex items-start justify-between gap-4">
        ...
        <data
          ...
          className="shrink-0 font-serif text-editorial-title font-normal leading-none tabular-nums text-cream"
        >
      ...
      <h3
        id={titleId}
        className="mt-4 overflow-anywhere font-serif text-editorial-title font-normal leading-editorial-title tracking-display"
      >
        {wine.name}
      </h3>
      ...
      <p
        className={`mt-4 font-serif text-wine-note font-normal leading-wine-note ${
          gifted ? 'text-cream/70' : 'text-cellar-muted'
        }`}
      >
        {wine.description}
      </p>
      <p className="mt-4 overflow-anywhere text-caption font-bold leading-label-relaxed text-cream/72">
