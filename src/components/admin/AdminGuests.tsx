...
export function AdminGuests({
  ...
  return (
    <section aria-labelledby="admin-page-title">
      ...
      <CreateFamilyDialog open={createOpen} busy={pendingFamilies.has('create')} error={createError} onCancel={() => setCreateOpen(false)} onCreate={async (values) => { await pendingFamilies.run('create', async (command) => { setCreateError(null); try { const result = await createFamily({ token, ...values }); if (!command.isCurrent()) return; if (result.kind === 'saved') { setCreateOpen(false); setExpanded((current) => new Set(current).add(result.family.id)); if (command.isLatest()) setFeedback('Família criada.') } else if ('message' in result) setCreateError(result.message); else onUnauthorized() } catch { if (command.isCurrent()) setCreateError('Não foi possível criar a família. Tente novamente.') } }) }} />
