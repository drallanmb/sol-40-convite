  ...
  return (
    ...
    >
      <div className="mx-auto grid max-w-[1320px] gap-[clamp(40px,6vw,90px)] md:grid-cols-2 md:items-center">
        <div>
          <p className="text-caption font-bold uppercase tracking-label text-peach">{DRESS.kicker}</p>
          ...
          <p className="mt-5 max-w-[62ch] text-body text-cream/80">{DRESS.intro}</p>
          ...
          <div className="mt-10 divide-y divide-cream/25 border-t border-cream/25">
            {DRESS.rules.map((rule) => (
              <div key={rule.audience} className="grid gap-1 py-6 sm:grid-cols-[minmax(100px,0.36fr)_1fr] sm:gap-6">
                <p className="font-serif text-subheading text-peach">{rule.audience}</p>
                <p className="text-body text-cream/80">{rule.text}</p>
              </div>
            ))}
          </div>
          ...
          <aside className="mt-9 bg-coral px-7 py-6 text-ink">
            ...
            <p className="mt-2 font-serif text-subheading">{DRESS.callout.lead}</p>
            <p className="mt-2 text-body">{DRESS.callout.text}</p>
          </aside>
        </div>
        ...
        <div aria-label="Referências de traje em branco e off-white" className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {DRESS.gallery.map((item) => (
            ...
            >
              ...
              />
              <figcaption className="absolute inset-x-3 bottom-3 bg-cream px-3 py-2 text-caption font-bold uppercase tracking-label text-plum">
                {item.caption}
              </figcaption>
            </figure>
          ))}
